...
A clickable frontend prototype of an AI-powered tax platform serving **clients and CPAs**, built for the GreenGrowth AI Engineer case study. This is a greenfield build: nothing existed beforehand, so the screens and the data model behind them are designed from scratch. The frontend is the deliverable: visual design, interaction design, and information architecture. The backend is deliberately fake: seeded data and an in-memory store, no real OCR / AI / auth.
...
- **Data:** `src/data/`, holding the typed domain model (`types.ts`), the hand-authored hero return (`hero.ts`), generated volume (`generate.ts`), assembled world (`world.ts`).
